...
sealed class ValidationError(val field: String, val message: String) {
    ...
    class EmailTooLong(val maxLength: Int) :
        ValidationError(
            "email",
    ...
    class PasswordTooShort(val minLength: Int) :
        ValidationError(
            "password",
            "Password must be at least $minLength characters"
        )

    class PasswordTooLong(val maxLength: Int) :
        ValidationError(
            "password",
            "Password must not exceed $maxLength characters"
        )

    class CurrentPasswordBlank :
        ValidationError(
            "currentPassword",
    ...
    class ContentTooLong(val maxLength: Int) :
        ValidationError(
            "content",
